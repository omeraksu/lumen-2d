...
class DielectricMaterial extends Material {
    ...
    computeScattering(ray, input_normal, t, contribution, worldAttenuation, wavelength) {
        ...
        */
        ...
        if(this.hasVolumeAbsorption) {
            if(dot > 0.0 && !refracted) {
                // keep absorption as is
            } else if (dot > 0.0 && refracted) {
                contribution.var -= this.volumeAbsorption[0];
                contribution.vag -= this.volumeAbsorption[1];
                contribution.vab -= this.volumeAbsorption[2];

                if(contribution.var < 0) contribution.var = 0;
                if(contribution.vag < 0) contribution.vag = 0;
                if(contribution.vab < 0) contribution.vab = 0;
            } else if (dot < 0.0 && refracted) {
                contribution.var += this.volumeAbsorption[0];
                contribution.vag += this.volumeAbsorption[1];
                contribution.vab += this.volumeAbsorption[2];
